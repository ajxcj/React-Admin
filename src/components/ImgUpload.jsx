
import { PlusOutlined, LoadingOutlined } from '@ant-design/icons';
import { message, Upload } from 'antd';
import { useState } from 'react';
import Cloud from 'leancloud-storage'

const getBase64 = (img, callback) => {//将本地图片资源转为base64编码
    const reader = new FileReader();
    reader.addEventListener('load', () => callback(reader.result));
    reader.readAsDataURL(img);
};

const beforeUpload = (file) => {//限制文件类型、大小
    const isJpgOrPng = file.type === 'image/jpeg' || file.type === 'image/png';

    if (!isJpgOrPng) {
        message.error('You can only upload JPG/PNG file!');
    }

    const isLt2M = file.size / 1024 / 1024 < 2;

    if (!isLt2M) {
        message.error('Image must smaller than 2MB!');
    }

    return isJpgOrPng && isLt2M;
};

const ImgUpload = (props) => {
    const [loading, setLoading] = useState(false);
    const [imageUrl, setImageUrl] = useState();
    // console.log('ImgUpload组件的props', props);//此处的props提供了onchange和value

    let handleCustom = (info) => {//自定义上传
        // console.log(info);
        getBase64(info.file, (base64) => {
            // console.log(base64);
            setLoading(true);
            const file = new Cloud.File('img.png', { base64 });//构建leancloud文件对象
            file.save().then(res => {//向leancloud提交并保存图片
                console.log(res);
                let { url } = res.attributes
                props.onChange(url)//给调用ImgUpload的Form表单提供值
                setImageUrl(url);
                setLoading(false);
            })

        })
    }

    const uploadButton = (
        <div>
            {loading ? <LoadingOutlined /> : <PlusOutlined />}
            <div
                style={{
                    marginTop: 8,
                }}
            >
                Upload
            </div>
        </div>
    );
    return (
        <Upload
            name="avatar"
            listType="picture-card"
            className="avatar-uploader"
            showUploadList={false}
            beforeUpload={beforeUpload}
            customRequest={handleCustom}
        >
            {imageUrl || props.value ? (
                <img
                    src={imageUrl || props.value}
                    alt="avatar"
                    style={{
                        width: '100%',
                    }}
                />
            ) : (
                uploadButton
            )}
        </Upload>
    );
};

export default ImgUpload;